...
class TCPConnection {
  private:
    ...
    void set_ack(TCPSegment& seg) const;

    bool in_end() const { return !_receiver.unassembled_bytes() && _receiver.stream_out().input_ended(); }

    bool out_end() const { return _sender.stream_in().eof() && _sender.next_seqno_absolute() == _sender.stream_in().bytes_written() + 2 && !_sender.bytes_in_flight(); }
